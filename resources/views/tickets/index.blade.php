            ...
            <div class="p-6 border-b border-gray-200">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                        <input type="text" placeholder="Cari ticket..."
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="ticketsTable">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ticket #</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Access Point</th>
                            <th
                                class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden lg:table-cell">
                                Lokasi</th>
                            <th
                                class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden md:table-cell">
                                Kategori</th>
                        ...
                        @forelse($tickets as $ticket)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-3 text-sm text-gray-900">
                                    <div class="max-w-[150px] truncate">{{ $ticket->accessPoint->name }}</div>
                                </td>
                                <td class="px-3 py-3 text-sm text-gray-900 hidden lg:table-cell">
                                    <div class="max-w-[180px]">
                                        <div class="truncate">{{ $ticket->accessPoint->room->floor->building->name }}</div>
                                ...
                                <td class="px-3 py-3 text-sm text-gray-900 hidden md:table-cell">
                                    <div class="max-w-[120px] truncate">{{ $ticket->category }}</div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900">
                                    <div class="flex items-center">
                                        <div
                                            class="w-8 h-8 bg-teal-500 rounded-full flex items-center justify-center text-white text-xs font-bold mr-2 flex-shrink-0">
                                            {{ substr($ticket->admin->name, 0, 2) }}
                                        </div>
                                        <div class="min-w-0">
                                            <div class="font-medium truncate max-w-[100px]">{{ $ticket->admin->name }}
                                            </div>
                                            <div class="text-xs text-gray-500 truncate max-w-[100px]">
                                                {{ $ticket->admin->unit_kerja }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-sm">
                                    <a href="{{ route('tickets.show', $ticket) }}"
                                        class="text-teal-600 hover:text-teal-900 font-medium text-xs">
                                        Detail →
                                    </a>
                                </td>
                            </tr>
