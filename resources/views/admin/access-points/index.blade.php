                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Posisi (X, Y)</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Open
                                Tickets</th>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center mr-3"
                                            ...
                                            <svg class="w-5 h-5" style="color: {{ $ap->status_color }};" fill="none"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form method="POST" action="{{ route('admin.access-points.status', $ap) }}"
                                        ...
                                        <select name="status" onchange="this.form.submit()"
                                            ...
                                            <option value="active" {{ $ap->status === 'active' ? 'selected' : '' }}>Normal
                                            </option>
                                            <option value="offline" {{ $ap->status === 'offline' ? 'selected' : '' }}>
                                                Bermasalah</option>
                                            <option value="maintenance"
                                                {{ $ap->status === 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                        </select>
                                    </form>
                                </td>
    ...
    <script>
        ...
        function confirmUpdateAPStatus(apId, apName) {
            Swal.fire({
                ...
                html: `
            <div class="text-left">
                <p class="text-sm text-gray-700 mb-3">Update status untuk <strong>${apName}</strong></p>
                <label class="block text-sm font-medium text-gray-700 mb-2">Pilih Status Baru:</label>
                <select id="new-ap-status" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
        ...
        `,
                ...
                reverseButtons: true,
                preConfirm: () => {
                    return document.getElementById('new-ap-status').value;
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = `/access-points/${apId}/update-status`;
