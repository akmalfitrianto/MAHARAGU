    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.buildings.create') }}"
                ...
                <svg class="w-5 h-5 mr-2 text-gray-400 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                Tambah Gedung
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse($buildings as $building)
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full">
                    ...
                    <div class="relative h-28 p-6 flex flex-col justify-end overflow-hidden"
                         style="background: linear-gradient(135deg, {{ $building->color ?? '#3b82f6' }}, {{ $building->color ? $building->color.'DD' : '#2563eb' }});">
                        
                        <div class="absolute -right-6 -top-6 opacity-20 transform rotate-12 group-hover:scale-110 transition-transform duration-500">
                            <svg class="w-32 h-32 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M19 2H5c-1.103 0-2 .897-2 2v16c0 1.103.897 2 2 2h14c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zM5 20V4h14l.002 16H5z"></path></svg>
                        </div>

                        <div class="relative z-10">
                            <h3 class="text-xl font-bold text-white leading-tight text-shadow-sm mb-1 truncate" title="{{ $building->name }}">
                                {{ $building->name }}
                            </h3>
                    ...
                    <div class="px-6 py-5 flex-1 flex flex-col gap-5">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="bg-gray-50 rounded-lg p-2.5 border border-gray-100 text-center">
                                <span class="block text-gray-400 text-[10px] font-bold uppercase tracking-wider">Ruangan</span>
                                <span class="block text-lg font-bold text-gray-800">{{ $building->total_rooms }}</span>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-2.5 border border-gray-100 text-center">
                                <span class="block text-gray-400 text-[10px] font-bold uppercase tracking-wider">Total AP</span>
                                <span class="block text-lg font-bold text-gray-800">{{ $building->total_access_points }}</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 mt-auto">
                            <div class="flex justify-between text-[11px] font-medium text-gray-500">
                                ...
                                @if($building->offline_access_points > 0)
                                    <span class="text-red-600 font-bold flex items-center">
                                        ...
                                        {{ $building->offline_access_points }} Offline
                                    </span>
                                @elseif($building->maintenance_access_points > 0)
                                    <span class="text-yellow-600 font-bold flex items-center">
                                        <span class="w-1.5 h-1.5 bg-yellow-500 rounded-full mr-1.5"></span>
                                        Maintenance
                                    </span>
                                @elseif($building->total_access_points > 0)
                            ...
                            <div class="flex h-1.5 w-full rounded-full bg-gray-100 overflow-hidden">
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between gap-3">
                        <a href="{{ route('admin.buildings.show', $building) }}" 
                           class="flex-1 inline-flex justify-center items-center px-4 py-2 bg-white border border-gray-200 text-gray-700 text-sm font-bold rounded-lg hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 transition-all shadow-sm group/btn">
                            Lihat Detail
                            <svg class="w-4 h-4 ml-1.5 text-gray-400 group-hover/btn:text-teal-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                        </a>
                        
                        <div class="flex items-center space-x-1">
                            <a href="{{ route('admin.buildings.edit', $building) }}" 
                               class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit Data">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            
                            <button type="button" onclick="confirmDeleteBuilding({{ $building->id }}, '{{ $building->name }}')"
                                    class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus Gedung">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                            
                            <form id="delete-building-{{ $building->id }}" action="{{ route('admin.buildings.destroy', $building) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                        </div>
                    </div>
            ...
            @empty
                <div class="col-span-full py-16 flex flex-col items-center justify-center text-center">
                    <div class="bg-gray-100 rounded-full p-6 mb-4">
                        <svg class="h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                    </div>
                    <h3 class="text-xl font-bold text-gray-900">Belum ada gedung</h3>
                    <p class="text-gray-500 mt-2 mb-8 max-w-sm">Data gedung diperlukan untuk memetakan lantai dan menempatkan access point.</p>
                    <a href="{{ route('admin.buildings.create') }}" class="px-6 py-3 bg-teal-600 text-white font-bold rounded-xl hover:bg-teal-700 transition shadow-lg shadow-teal-600/20">
