        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="shape_type" 
                            ...
                            required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                        <option value="rectangle" {{ $room->shape_type === 'rectangle' ? 'selected' : '' }}>Rectangle (Persegi Panjang)</option>
                        <option value="square" {{ $room->shape_type === 'square' ? 'selected' : '' }}>Square (Persegi)</option>
                        <option value="l_shape" {{ $room->shape_type === 'l_shape' ? 'selected' : '' }}>L-Shape</option>
                        <option value="u_shape" {{ $room->shape_type === 'u_shape' ? 'selected' : '' }}>U-Shape</option>
                        <option value="custom" {{ $room->shape_type === 'custom' ? 'selected' : '' }}>Custom SVG Path</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" 
                               ...
                               max="1000" 
                               required
                               value="{{ old('position_x', $room->position_x) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <input type="number" 
                               ...
                               max="600" 
                               required
                               value="{{ old('position_y', $room->position_y) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="bg-gray-50 rounded-lg p-4 border-2 border-gray-300">
                    <svg id="previewCanvas" viewBox="0 0 1000 600" class="w-full" style="height: 400px;">
                        <defs>
                ...
                <div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                    ...
                    <p class="text-xs">Ruangan ini memiliki {{ $room->accessPoints()->count() }} Access Point. Mengubah ukuran atau posisi akan mempengaruhi tampilan AP di denah.</p>
                </div>
                @endif
            </div>
        </div>
        ...
        updatePreview() {
            ...
            switch(this.shape_type) {
                ...
                case 'l_shape':
                    ...
                    break;
                case 'u_shape':
                    this.previewPath = `M ${x} ${y} L ${x + w} ${y} L ${x + w} ${y + h} L ${x + w - (w * 0.35)} ${y + h} L ${x + w - (w * 0.35)} ${y + (h * 0.5)} L ${x + (w * 0.35)} ${y + (h * 0.5)} L ${x + (w * 0.35)} ${y + h} L ${x} ${y + h} Z`;
                    break;
                case 'custom':
